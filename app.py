...
# ✅ Handle Form Submission (Trigger Worker Process)
@app.post("/", response_class=HTMLResponse)
async def generate_speech(request: Request, text: str = Form(...)):
    try:
        # Save the input text to a file (Worker will pick it up)
        ...
        # Wait until output is generated by the worker
        while not os.path.exists(output_file):
            pass  # Keep checking until worker generates the file

        # Provide download link
        download_url = f"/download/{os.path.basename(output_file)}"
        return html_template.replace(
            "{% if download_url %}...{% endif %}",
